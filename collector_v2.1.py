...
class Collector:
    ...
    def print_kospi_stock_data(self, stock_name, day):
        ...
        # 종목 이름을 받아와서 해당 종목의 코드 값을 찾아줌
        stock_number = self.item.code_df_kospi['code'][self.item.code_df_kospi['code_name'] == stock_name].iloc[0]
        print('{code_name}의 종목 코드는 {code} 입니다'.format(code_name=stock_name, code=stock_number))

        # 아래 api의 함수들은 ctrl + 클릭해서 들어가보면 사용 방법 설명 되어 있음.
        # self.api : openapi클래스의 인스턴스
        # get_total_data : Openapi클래스의 메서드
        total_data = self.api.get_total_data(stock_number, day)
        print("total_data: !!!")
        print(total_data)

        # get_one_day_option_data : Openapi클래스의 메서드
        one_data = self.api.get_one_day_option_data(stock_number, day, 'close')
        print("one_data: !!!")
        print('{day}의 종가는 {one_data}입니다.'.format(day=day, one_data=one_data))
        print("----------------\n")


    def print_kosdaq_stock_data(self, stock_name, day):
        print("get_stock_data 함수에 들어왔습니다.")
        ...
        # 종목 이름을 받아와서 해당 종목의 코드 값을 찾아줌
        stock_number = self.item.code_df_kosdaq['code'][self.item.code_df_kosdaq['code_name'] == stock_name].iloc[0]
        print('{code_name}의 종목 코드는 {code} 입니다'.format(code_name=stock_name, code=stock_number))
...
if __name__ == "__main__":
    ...
    c = Collector()
    c.print_kospi_stock_data('삼성전자', '20210304')
    c.print_kosdaq_stock_data('유바이오로직스', '20210304')
